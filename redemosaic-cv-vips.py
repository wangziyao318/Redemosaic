...
if __name__ == "__main__":
    ...
    rgbimg = pyvips.Image.new_from_file("1.TIF").numpy()
    print("pyvipsread\t {:.3f}s".format(time.time()-start))
    
    cfaimg = mosaic(rgbimg, pattern)
    print("mosaic\t {:.3f}s".format(time.time()-start))

    pyvips.Image.new_from_array(cfaimg, interpretation="rgb").write_to_file("cfa.png")
    ...
    pyvips.Image.new_from_array(newimg, interpretation="rgb").write_to_file("re.png")
