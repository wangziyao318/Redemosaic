...
if __name__ == "__main__":
    ...
    psnr_T = np.array(psnrs_real, dtype=np.float64)
    psnr_F = np.array(psnrs_fake, dtype=np.float64)
    X_psnr = np.concatenate([psnr_T, psnr_F], axis=0)

    ssim_T = np.array(ssims_real, dtype=np.float64)
    ssim_F = np.array(ssims_fake, dtype=np.float64)
    X_ssim = np.concatenate([ssim_T, ssim_F], axis=0)

    vmaf_FHD_T = np.array(vmafs_FHD_real, dtype=np.float64)
    vmaf_FHD_F = np.array(vmafs_FHD_fake, dtype=np.float64)
    X_vmaf_FHD = np.concatenate([vmaf_FHD_T, vmaf_FHD_F], axis=0)

    vmaf_4K_T = np.array(vmafs_4K_real, dtype=np.float64)
    vmaf_4K_F = np.array(vmafs_4K_fake, dtype=np.float64)
    ...
    # clf_MLP = MLPClassifier(hidden_layer_sizes=[15,5], random_state=1, max_iter=1000)
